...
#include "MK4duo.h"
#include "Mk4duoVersion.h"
#include "SITestBanco.h"

void syncWithEsp()
{
  //Search for fw line
  char espSerialBuffer[128];
  do
  {
    //Wait half sec
    delay(500);
    //Write firmware version
    SerialEsp.print("SCRIBITFW:");
    SerialEsp.println(SCRIBIT_FW_VER);
    SerialEsp.readBytesUntil(0x0A, espSerialBuffer, 128);
  } while (strstr(espSerialBuffer, "SCRIBITSTART") == nullptr);
}

void checkAndRunTest()
{
  //Set pin pullup
  pinMode(X_MAX_TEST, INPUT_PULLUP);
  delayMicroseconds(10); //Wait for set-up time
  if (digitalRead(X_MAX_TEST) == LOW)
  {
    SITestBanco testMode;
    testMode.run();
  }
  //Disable pullup
  pinMode(X_MAX_TEST, INPUT);
}
void setup()
{
#ifndef SIMK_DEBUG_BUILD
  checkAndRunTest();
#endif
  printer.setup();
  SerialEsp.println();
