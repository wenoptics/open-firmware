...
class HAL {
  ...
  public: /** Public Function */
    ...
    FORCE_INLINE static void pinMode(const pin_t pin, const uint8_t mode) {
      //Check for pin 26
    if (pin == 26 && (mode==OUTPUT || mode == OUTPUT_LOW || mode == OUTPUT_HIGH))
    {
      SERIAL_LM(ER, "Trying to command thermistor pin. HAL::pinMode");
      return;
    }
      switch (mode) {
